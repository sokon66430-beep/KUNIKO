...
export function PosShiftModal({ terminal, initialAction, onClose }: { terminal: string; initialAction?: ShiftAction; onClose: () => void }) {
  ...
  // own header (and the Back button) aren't hidden behind the bar.
  ...
  async function submitMovement(payload: { amountUsd: number; amountRiel: number; reason: string; notes?: string; reference?: string; bagColor?: string }) {
    if (!current) return;
  ...
  return (
    <>
      {/* Full-screen page over the till. On a till it sits below the fixed bar
          (top-[52px]) so this screen's own header stays visible and tappable. */}
